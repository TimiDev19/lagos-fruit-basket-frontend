...
const getDeliveryMultiplier = (cart: CartItemType[]): number => {
  ...
  for (const item of cart) {
    const m = HEAVY_ITEM_MULTIPLIERS[item.name];
    if (m && m > heavyMultiplier) heavyMultiplier = m;
  }

  return Math.max(quantityMultiplier, heavyMultiplier);
};
...
const CartModal = () => {
  ...
  const baseDeliveryFee =
    fulfillmentType === "delivery"
      ? DELIVERY_FEES[selectedLocation] ?? 0
      : 0;
  ...
  const sendOrder = () => {
    ...
    if (!customerEmail || !customerPhone) return toast("Fill required fields");
    if (!fulfillmentType) return toast("Select delivery or pickup");

    if (
      fulfillmentType === "delivery" &&
      (!selectedLocation || !customerAddress)
    ) {
      return toast("Complete delivery details");
    }
